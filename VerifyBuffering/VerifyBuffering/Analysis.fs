...
let ExpensiveObject = "DspVector"
let IgnoreErrorWithConstantSizeLessThan = 10

type BufferVariable(name: string, isChild: bool, isIteration: bool) = class
    member x.Name = name
    member x.IsChild = isChild
    member x.IsIteration = isIteration
end
...
let Analyze(tree: SyntaxTree) =
    ...
    let VerifyBufferArgument(argument: Expression): List<string> =
        match argument with
        _ -> []
    let VerifyBufferArguments(methodCall: InvocationExpression): List<string> =
        methodCall.Arguments |> Seq.collect VerifyBufferArgument |> Seq.toList
        ...
        let thisResult = 
            ...
            | :? CSharpModifierToken as modfierToken
                ...
                -> 
                    ...
                    | _ -> ()
                    []
            | :? InvocationExpression as invocation
                -> VerifyBufferArguments(invocation)
            | _ -> []
